    ...
    def load_existing_token(self):
        ...
        try:
            ...
            if token and expires_at:
                ...
                self.logger.info(f"기존 토큰 로드 완료 (만료: {datetime.fromtimestamp(expires_at).strftime('%Y-%m-%d %H:%M:%S')})")
            else:
                self.logger.warning("토큰 파일이 유효하지 않음")

        except FileNotFoundError:
            self.logger.info("토큰 파일 없음 - 새로 발급 필요")
